...
class LossyFdAdapter {
   ...
   public:
    ...
    explicit LossyFdAdapter(AdapterT&& adapter)
        : _adapter(std::move(adapter)) {}

    //! \brief Read from the underlying AdapterT instance, potentially dropping
    //! the read datagram \returns std::optional<TCPSegment> that is empty if
    //! the segment was dropped or if
    ...
    //! \brief Write to the underlying AdapterT instance, potentially dropping
    //! the datagram to be written \param[in] seg is the packet to either write
    //! or drop
