...
class TCPPeer {
    ...
    ByteStream outbound_stream_{cfg_.send_capacity},
   ...
   public:
    ...
    bool has_ackno() const {
        return receiver_.send(inbound_stream_.writer()).ackno.has_value();
    }
    ...
    bool active() const {
        ...
        return (not outbound_stream_.reader().is_finished()) or
               ...
               sender_.sequence_numbers_in_flight();
    }
    ...
    void receive(TCPSegment seg) {
        ...
        receiver_.receive(std::move(seg.sender_message), reassembler_,
    ...
    std::optional<TCPSegment> maybe_send() {
        ...
        // Get (possible) outgoing TCPSenderMessage, using empty message if we
