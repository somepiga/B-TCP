...
void TCPMinnowSocket<AdaptT>::_initialize_TCP(const TCPConfig& config) {
    ...
    // 有四种可能的事件需要处理：
    // 1) 接收到的入站数据报（需要传递给 TCPConnection::segment_received 方法）
    // 2) 通过 write() 调用从本地应用程序接收到的出站字节
    //    (需要从本地流套接字中读取,并传递给 TCPConnection::data_written 方法)
    // 3) 由 TCPConnection 重新组装后的入站字节
    //    (需要从 inbound_stream 中读取，并写回本地流套接字,以返回给应用程序)
    // 4) TCP 生成的出站段（需要传递给底层的数据报套接字）
...
TCPMinnowSocket<AdaptT>::TCPMinnowSocket(AdaptT&& datagram_interface)
    : TCPMinnowSocket(socket_pair_helper(SOCK_STREAM),
...
void CS144TCPSocket::connect(const Address& address) {
    ...
    multiplexer_config.source = {"169.254.144.9",
                                 to_string(uint16_t(random_device()()))};
...
EthernetAddress random_private_ethernet_address() {
    ...
    addr.at(0) |= 0x02;  // "10" in last two binary digits marks a private
                         // Ethernet address
    addr.at(0) &= 0xfe;
...
void FullStackSocket::connect(const Address& address) {
    ...
    FdAdapterConfig multiplexer_config;
    multiplexer_config.source = {LOCAL_TAP_IP_ADDRESS,
                                 to_string(uint16_t(random_device()()))};
    multiplexer_config.destination = address;

    TCPOverIPv4OverEthernetMinnowSocket::connect(tcp_config,
                                                 multiplexer_config);
}
