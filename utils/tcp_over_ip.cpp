...
optional<TCPSegment> TCPOverIPv4Adapter::unwrap_tcp_in_ip(
    const InternetDatagram& ip_dgram) {
    // is the IPv4 datagram for us?
    // Note: it's valid to bind to address "0" (INADDR_ANY) and reply from
    // actual address contacted
    ...
    TCPSegment tcp_seg;
    if (not parse(tcp_seg, ip_dgram.payload,
                  ip_dgram.header.pseudo_checksum())) {
        return {};
    }
    ...
    // should we target this source addr/port (and use its destination addr as
    // our source) in reply?
    if (listening()) {
        if (tcp_seg.sender_message.SYN and not tcp_seg.reset) {
            config_mutable().source =
                Address{inet_ntoa({htobe32(ip_dgram.header.dst)}),
                        config().source.port()};
            config_mutable().destination =
                Address{inet_ntoa({htobe32(ip_dgram.header.src)}),
                        tcp_seg.udinfo.src_port};
            set_listening(false);
        } else {
            return {};
        }
