...
//! 把 Raw 转为 `const sockaddr *`.
Address::Raw::operator const sockaddr*() const {
    return reinterpret_cast<const sockaddr*>(&storage);
}

//! \param[in] addr 是 socket address
//! \param[in] size 是 `addr` 的大小
...
class gai_error_category : public error_category {
   public:
    ...
    //! \param[in] return_value the error return value from
    //! [getaddrinfo(3)](\ref man3::getaddrinfo)
...
Address::Address(const string& node,
    ...
    // put resolved_address in a wrapper so it will get freed if we have to
    // throw an exception
...
pair<string, uint16_t> Address::ip_port() const {
    ...
    const int gni_ret = getnameinfo(
        static_cast<const sockaddr*>(_address), _size, ip.data(), ip.size(),
