...
//! [IPv4](@ref man7::ip) 和 DNS 操作的包装.
class Address {
   public:
    //! \brief 包装 [sockaddr_storage](@ref man7::socket).
    //! \details sockaddr_storage 足以存储任何套接字地址（IPv4 或 IPv6）.
    class Raw {
       public:
   ...
   private:
    socklen_t _size;  //!< 包装地址大小
    Raw _address{};   //!< 包装类 [sockaddr_storage](@ref man7::socket)
                      //!< 包含地址.
