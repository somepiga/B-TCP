...
//! \param[in] next_hop IP address of the next hop (typically a router or
//! default gateway)
TCPOverIPv4OverEthernetAdapter::TCPOverIPv4OverEthernetAdapter(
    ...
    : _tap(move(tap)),
      _interface(eth_address, ip_address),
      _next_hop(next_hop) {
    // Linux seems to ignore the first frame sent on a TAP device, so send a
    // dummy frame to prime the pump :-(
