...
EventLoop::BasicRule::BasicRule(size_t s_category_id,
...
EventLoop::Result EventLoop::wait_next_event(const int timeout_ms) {
    ...
    for (auto it = _fd_rules.begin();
        ...
        if (this_rule.cancel_requested) {
            //      this_rule.cancel();
            //      if rule is cancelled externally, no need to call the
            //      cancellation callback this makes it easier to cancel rules
            //      and delete captured objects right away
    ...
    for (auto [it, idx] = make_pair(_fd_rules.begin(), static_cast<size_t>(0));
        ...
        if (poll_error) {
            ...
            const int ret = getsockopt(this_rule.fd.fd_num(), SOL_SOCKET,
                                       SO_ERROR, &socket_error, &optlen);
        ...
        if (poll_hup && ((this_pollfd.events && !poll_ready) or
            ...
            // if we asked for the status, and the _only_ condition was a
            // hangup, this FD is defunct:
            //   - if it was POLLIN and nothing is readable, no more will ever
            //   be readable
        ...
        if (poll_ready) {
            // we only want to call callback if revents includes the event we
            // asked for
