...
#include "byte_stream_test_harness.h"
#include "common.h"
#include "connect/reassembler.h"

using StreamAndReassembler = std::pair<StreamBuffer, Reassembler>;

template <std::derived_from<TestStep<StreamBuffer>> T>
 ...
 public:
  ReassemblerTestHarness(std::string test_name, uint64_t capacity)
      : TestHarness(std::move(test_name),
                    ...
                    {StreamBuffer{capacity}, Reassembler{}}) {}

  template <std::derived_from<TestStep<StreamBuffer>> T>
  void execute(const T& test) {
    TestHarness<StreamAndReassembler>::execute(
        ReassemblerByteStreamTestStep{test});
  }
...
struct BytesPending : public ExpectNumber<StreamAndReassembler, uint64_t> {
  ...
  uint64_t value(StreamAndReassembler& sr) const override {
    return sr.second.bytes_pending();
  }
};
...
struct Insert : public Action<StreamAndReassembler> {
  ...
  Insert(std::string data, uint64_t first_index)
      : data_(std::move(data)), first_index_(first_index) {}
  ...
  void execute(StreamAndReassembler& sr) const override {
    sr.second.insert(first_index_, data_, is_last_substring_,
                     sr.first.writer());
  }
};
