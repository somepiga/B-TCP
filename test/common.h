...
class ExpectationViolation : public std::runtime_error {
 public:
  ...
  template <typename T>
  inline ExpectationViolation(const std::string& property_name,
                              const T& expected, const T& actual);
};

template <typename T>
ExpectationViolation::ExpectationViolation(const std::string& property_name,
                                           const T& expected, const T& actual)
    : ExpectationViolation{"The object should have had " + property_name +
                           " = " + to_string(expected) +
                           ", but instead it was " + to_string(actual) + "."} {}

template <>
inline ExpectationViolation::ExpectationViolation(
    const std::string& property_name, const bool& expected, const bool& actual)
    : ExpectationViolation{"The object should have had " + property_name +
                           " = " + boolstr(expected) + ", but instead it was " +
                           boolstr(actual) + "."} {}
...
class TestHarness {
 ...
 protected:
  explicit TestHarness(std::string test_name, std::string_view desc, T&& object)
    ...
    steps_executed_.emplace_back("Initialized " + demangle(typeid(T).name()) +
                                     " with " + std::string{desc},
                                 Printer::def);
  }
 ...
 public:
  void execute(const TestStep<T>& step) {
    try {
      ...
      pr_.diagnostic(test_name_, steps_executed_, step.str(), e);
      throw std::runtime_error{"The test \"" + test_name_ +
                               "\" made your code throw an exception."};
    }
  }
};
