...
#include "bidirectional_stream_copy.h"
#include "exception.h"
#include "router.h"
#include "tcp_minnow_socket.cpp"
#include "tcp_over_ip.h"
...
EthernetAddress random_host_ethernet_address() {
    ...
    addr.at(0) |= 0x02;  // 最后两位=10：标记为私有以太网地址
    addr.at(0) &= 0xfe;
...
class NetworkInterfaceAdapter : public TCPOverIPv4Adapter {
   private:
    ...
    Address _next_hop;
    pair<FileDescriptor, FileDescriptor> _data_socket_pair =
        socket_pair_helper(SOCK_DGRAM);
   ...
   public:
    ...
    optional<TCPSegment> read() {
        ...
        // Give the frame to the NetworkInterface. Get back an Internet datagram
        // if frame was carrying one.
...
class TCPSocketEndToEnd : public TCPMinnowSocket<NetworkInterfaceAdapter> {
   ...
   public:
    ...
    void connect(const Address& address) {
        ...
        _local_address = Address{_local_address.ip(),
                                 uint16_t(random_device()())};  // 加个端口号
        ...
        TCPMinnowSocket<NetworkInterfaceAdapter>::connect({},
                                                          multiplexer_config);
    }
...
void program_body(bool is_client,
                  const string& bounce_host,
                  const string& bounce_port,
                  const bool debug) {
    ...
    if (is_client) {
        host_side = router.add_interface(
            {random_router_ethernet_address(), Address{"192.168.0.1"}});
        internet_side = router.add_interface(
            {random_router_ethernet_address(), Address{"10.0.0.192"}});
        router.add_route(Address{"192.168.0.0"}.ipv4_numeric(), 16, {},
                         host_side);
        router.add_route(Address{"10.0.0.0"}.ipv4_numeric(), 8, {},
                         internet_side);
        router.add_route(Address{"172.16.0.0"}.ipv4_numeric(), 12,
                         Address{"10.0.0.172"}, internet_side);
    } else {
        host_side = router.add_interface(
            {random_router_ethernet_address(), Address{"172.16.0.1"}});
        internet_side = router.add_interface(
            {random_router_ethernet_address(), Address{"10.0.0.172"}});
        router.add_route(Address{"172.16.0.0"}.ipv4_numeric(), 12, {},
                         host_side);
        router.add_route(Address{"10.0.0.0"}.ipv4_numeric(), 8, {},
                         internet_side);
    ...
    thread network_thread([&]() {
        try {
            ...
            event_loop.add_rule(
                ...
                Direction::In, [&] {
                    auto frame_opt =
                        maybe_receive_frame(sock.adapter().frame_fd());
                    if (not frame_opt) {
                        return;
                    }
                    EthernetFrame frame = move(frame_opt.value());
            ...
            event_loop.add_rule(
                "frames from router to Internet", internet_socket,
                Direction::Out,
                [&] {
            ...
            event_loop.add_rule(
                "frames from Internet to router", internet_socket,
                Direction::In, [&] {
            ...
            while (true) {
                ...
                while (auto frame =
                           router.interface(internet_side).maybe_send()) {
