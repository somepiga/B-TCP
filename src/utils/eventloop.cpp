...
EventLoop::BasicRule::BasicRule(size_t s_category_id, InterestT s_interest,
                                CallbackT s_callback)
    : category_id(s_category_id),
      interest(std::move(s_interest)),
      callback(std::move(s_callback)) {}

EventLoop::FDRule::FDRule(BasicRule&& base, FileDescriptor&& s_fd,
                          Direction s_direction, CallbackT s_cancel,
                          InterestT s_recover)
    : BasicRule(base),
      fd(std::move(s_fd)),
...
EventLoop::RuleHandle EventLoop::add_rule(
