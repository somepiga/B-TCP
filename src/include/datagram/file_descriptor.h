...
class FileDescriptor {
  ...
  class FDWrapper {
   public:
    ...
    bool closed_ =
        false;  // Flag indicating whether FDWrapper::fd_ has been closed
    bool non_blocking_ =
        false;  // Flag indicating whether FDWrapper::fd_ is non-blocking
    unsigned read_count_ =
        0;  // The number of times FDWrapper::fd_ has been read
    unsigned write_count_ =
        0;  // The numberof times FDWrapper::fd_ has been written
 ...
 public:
  ...
  // FileDescriptor can be moved, but cannot be copied implicitly (see
  // duplicate())
  FileDescriptor(const FileDescriptor& other) =
      delete;  // copy construction is forbidden
  FileDescriptor& operator=(const FileDescriptor& other) =
      delete;  // copy assignment is forbidden
  FileDescriptor(FileDescriptor&& other) =
      default;  // move construction is allowed
  FileDescriptor& operator=(FileDescriptor&& other) =
      default;  // move assignment is allowed
};
